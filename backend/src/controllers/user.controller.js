...
const registerUser = asyncHandler( async(req, res) => {
    ...
    const avatar = await uploadOnCDN(avatarLocalPath);
    const coverImage = await uploadOnCDN(coverImageLocalPath);
...
const loginUser = asyncHandler( async(req,res) => {
    ...
    if(!user){
        throw new ApiError(404, "error doesnt exist");
    }
    ...
    const options = {
        httpOnly : true,
        secure : true // can only modify from  the server
    }
